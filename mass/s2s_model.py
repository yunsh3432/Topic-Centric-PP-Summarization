...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self,
                ...
                input_embeds=None,
                **unused):
        x, next_decoder_cache, inner_states, extra = self.extract_features(prev_output_tokens, encoder_out, incremental_state, input_embeds, **unused)
        x = self.output_layer(x)
        return x, next_decoder_cache, inner_states, extra

    def extract_features(self, prev_output_tokens, encoder_out=None, incremental_state=None, input_embeds=None, **unused):
        # embed positions
        ...
        if input_embeds is None:
            ...
            x = input_embeds * self.embed_scale
        ...
        for i, layer in enumerate(self.layers):
            inner_states += (x,)
            inc_state = incremental_state[i] if incremental_state is not None else None
            x, attn, layer_past = layer(
                x,
            ...
            next_decoder_cache = next_decoder_cache + (layer_past.copy(),)
        inner_states = [hidden_state.transpose(0, 1) for hidden_state in inner_states]

        # T x B x C -> B x T x C
        x = x.transpose(0, 1)

        return x, next_decoder_cache, inner_states, {'attn': attn},
